...
class TaskController extends Controller
{
    public function index(){
        $disk = FacadesStorage::disk(config('backup.backup.destination.disks')[0]);
    ...
    $dump = "mysqldump --host $db_host -u $db_user -p $db_pass --opt $db_name > $salida_sql";
    ...
    public function create()
    {
        ...
        try {
            ...
            FacadesArtisan::call('backup:run', ['--only-db' => 'true']);
    ...
    public function download($file_name)
    {
        ...
        $disk = FacadesStorage::disk(config('backup.backup.destination.disks')[0]);
        if ($disk->exists($file)) {
            $fs = FacadesStorage::disk(config('backup.backup.destination.disks')[0])->getDriver();
